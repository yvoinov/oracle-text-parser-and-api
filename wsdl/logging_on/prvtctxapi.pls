...
create or replace package body ctx_api is
 ...
 -- --------------------------------------------------------------
 procedure addlogentry (p_request in varchar2, p_api_funct in varchar2, 
                        p_api_params in varchar2 default null) is
 -- Logging procedure
 -- p_api_params for store thes_name and api call arguments
 begin
  insert into ctx_api_log
  values(sysdate, p_request, p_api_funct, p_api_params);
  commit;
 end addlogentry;
 -- --------------------------------------------------------------
 ...
 begin
  ...
  -- Log entry
  addlogentry (p_phrase, 'phrase_relation_exists',
               p_thes_name||','||v_relation);
 ...
 begin
  ...
  -- Log entry
  addlogentry (v_phrase, 'search_expansion_level', p_thes_name);
  loop
 ...
 begin
  v_phrase := trim(p_phrase); -- Trim leading and trailing spaces
  -- Log entry
  addlogentry (v_phrase, 'search_expansion_term', p_thes_name);
  loop
   v_init_level := v_init_level + 1; -- Increment level
   ctx_thes.bt(v_restab, v_phrase, v_init_level, p_thes_name); -- Get top BT for current level
   v_top_bt := v_restab(v_restab.last).xphrase; -- Get category for current level
 ...
 begin
  ...
  -- Log entry
  addlogentry (v_phrase, 'has_homographs', p_thes_name);
 ...
 begin
  ...
  -- Log entry
  addlogentry (v_phrase, 'get_qualifiers', p_thes_name);
 ...
 begin -- Check phrase for homographs if qualifier is absent
  v_phrase := trim(p_phrase); -- Trim leading and trailing spaces
  -- Log entry
  addlogentry (v_phrase, 'get_bt_function', p_thes_name||','||p_level);
 ...
 begin
  v_phrase := trim(p_phrase); -- Trim leading and trailing spaces
  -- Log entry
  addlogentry (v_phrase, 'get_bt_procedure', p_thes_name||','||p_level);
  ctx_thes.bt(v_res, v_phrase, p_level, p_thes_name); -- Get BT's level p_level
 ...
 begin -- Check phrase for homographs if qualifier is absent
  v_phrase := trim(p_phrase); -- Trim leading and trailing spaces
  -- Log entry
  addlogentry (v_phrase, 'get_nt', p_thes_name||','||p_level);
  if instr(v_phrase,'(') = 0 and instr(v_phrase,')') = 0 then
   if ctx_api.has_homographs (v_phrase, p_thes_name) then
    raise term_has_homographs;
   end if;
 ...
 begin -- Check phrase for homographs if qualifier is absent
  v_phrase := trim(p_phrase); -- Trim leading and trailing spaces
  -- Log entry
  addlogentry (v_phrase, 'get_ntp', p_thes_name||','||p_level);
  if instr(v_phrase,'(') = 0 and instr(v_phrase,')') = 0 then
   if ctx_api.has_homographs (v_phrase, p_thes_name) then
    raise term_has_homographs;
   end if;
 ...
 begin -- Check phrase for homographs if qualifier is absent
  v_phrase := trim(p_phrase); -- Trim leading and trailing spaces
  -- Log entry
  addlogentry (v_phrase, 'get_rt', p_thes_name);
  if instr(v_phrase,'(') = 0 and instr(v_phrase,')') = 0 then
   if ctx_api.has_homographs (v_phrase, p_thes_name) then
    raise term_has_homographs;
   end if;
 ...
 begin -- Check phrase for homographs if qualifier is absent
  v_phrase := trim(p_phrase); -- Trim leading and trailing spaces
  -- Log entry
  addlogentry (v_phrase, 'get_syn', p_thes_name);
  if instr(v_phrase,'(') = 0 and instr(v_phrase,')') = 0 then
   if ctx_api.has_homographs (v_phrase, p_thes_name) then
    raise term_has_homographs;
   end if;
 ...
 -- -----------------------------------------
  ...
  -- Log entry
  addlogentry (p_search_str, 'search_string_parser',
               p_thes_name||','||p_logical_op||','||p_query_opt||', Level:'||
               p_expansion_level||', Refine:'||p_refine_on||', Expansion: '||p_exp_detail_on);
 ...
 begin
  -- Log entry
  addlogentry ('CTX_CONTENT_API', 'term_counter', p_thes_name);
  select count(*)
  into v_count
 ...
 begin
  -- Log entry
  addlogentry ('CTX_CONTENT_API', 'thes_loaded', 'GET THES_LIST');
  select ths_name
 ...
 begin
  -- Log entry
  addlogentry (p_phrase, 'phrase_exists', p_thes_name);
